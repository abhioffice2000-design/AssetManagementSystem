...
export class AdminReportsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.requestStats.total === 0) {
      this.requestStats = { total: 24, pending: 5, approved: 8, rejected: 2, completed: 7, inProgress: 2 };
    }
  ...
  async loadPieChartData(): Promise<void> {
    ...
    try {
      ...
      for (const sub of allSubcategoryCounts) {
        ...
        if (!typeSubCatMap[typeKey]) typeSubCatMap[typeKey] = [];
      ...
      let fallbackIndex = 0;
      this.typeBreakdown = typeCounts.map((item: any) => {
  ...
  getPieChartGradient(): string {
    ...
    for (let i = 0; i < this.typeBreakdown.length; i++) {
      ...
      if (i === this.typeBreakdown.length - 1) end = 100;
  ...
  switchReportTab(tab: 'monthly' | 'yearly'): void { this.activeReportTab = tab; }

  getMaxRequested(): number {
    if (this.activeReportTab === 'monthly') return Math.max(...this.monthlyData.map(r => r.requested), 1);
    return Math.max(...this.yearlyData.map(r => r.requested), 1);
  }

  getBarWidth(value: number): number { return Math.round((value / this.getMaxRequested()) * 100); }
  ...
  loadTeamAssetHoldings(): void {
    this.teamAssetHoldings = [
      { team: 'Frontend',     department: 'Engineering',      lead: 'Arjun Reddy',   totalAssets: 12, hardware: 8, software: 3, peripheral: 1, totalValue: 485000, members: 6 },
      { team: 'Backend',      department: 'Engineering',      lead: 'Suresh Patel',  totalAssets: 10, hardware: 6, software: 3, peripheral: 1, totalValue: 420000, members: 5 },
      { team: 'UX Design',    department: 'Design',           lead: 'Meera Nair',    totalAssets: 8,  hardware: 4, software: 4, peripheral: 0, totalValue: 360000, members: 4 },
      { team: 'DevOps',       department: 'Engineering',      lead: 'Vikram Singh',  totalAssets: 6,  hardware: 3, software: 2, peripheral: 1, totalValue: 280000, members: 3 },
      { team: 'QA',           department: 'Engineering',      lead: 'Priya Sharma',  totalAssets: 5,  hardware: 3, software: 2, peripheral: 0, totalValue: 210000, members: 3 },
      { team: 'HR Operations',department: 'Human Resources',  lead: 'Ananya Desai',  totalAssets: 4,  hardware: 2, software: 1, peripheral: 1, totalValue: 150000, members: 3 }
  ...
  formatCurrency(value: number): string { return '₹' + value.toLocaleString('en-IN'); }
  ...
  getDummyAssets(): Asset[] {
    const now = new Date();
    const exp1 = new Date(now.getTime() + 30  * 86400000).toISOString();
    const exp2 = new Date(now.getTime() + 90  * 86400000).toISOString();
    return [
      { id: '1', assetTag: 'HW-001', name: 'MacBook Pro 16"',        type: AssetType.HARDWARE,   status: AssetStatus.ALLOCATED,  vendor: 'Apple',         cost: 185000, purchaseDate: '2023-01-15', warrantyExpiry: exp2, location: 'HQ',          category: 'Laptop',       subCategory: '', serialNumber: '', condition: AssetCondition.GOOD },
      { id: '2', assetTag: 'HW-002', name: 'Dell XPS 15',            type: AssetType.HARDWARE,   status: AssetStatus.AVAILABLE,  vendor: 'Dell',          cost: 145000, purchaseDate: '2023-03-20', warrantyExpiry: exp1, location: 'HQ',          category: 'Laptop',       subCategory: '', serialNumber: '', condition: AssetCondition.NEW  },
      { id: '3', assetTag: 'HW-003', name: 'Lenovo ThinkPad',        type: AssetType.HARDWARE,   status: AssetStatus.IN_REPAIR,  vendor: 'Lenovo',        cost: 120000, purchaseDate: '2022-11-10', warrantyExpiry: exp1, location: 'Remote',      category: 'Laptop',       subCategory: '', serialNumber: '', condition: AssetCondition.DAMAGED },
      ...
      { id: '5', assetTag: 'NW-001', name: 'Cisco Router 4000',      type: AssetType.NETWORK,    status: AssetStatus.ALLOCATED,  vendor: 'Cisco',         cost: 85000,  purchaseDate: '2022-05-12', warrantyExpiry: exp2, location: 'Server Room', category: 'Networking',   subCategory: '', serialNumber: '', condition: AssetCondition.GOOD },
      { id: '6', assetTag: 'PR-001', name: 'Dell UltraSharp 27"',    type: AssetType.PERIPHERAL, status: AssetStatus.ALLOCATED,  vendor: 'Dell',          cost: 35000,  purchaseDate: '2023-06-10', warrantyExpiry: '2026-06-10', location: 'HQ',  category: 'Monitor',    subCategory: '', serialNumber: '', condition: AssetCondition.GOOD },
      { id: '7', assetTag: 'FN-001', name: 'Herman Miller Chair',    type: AssetType.FURNITURE,  status: AssetStatus.RESERVED,   vendor: 'Herman Miller', cost: 110000, purchaseDate: '2021-12-05', warrantyExpiry: '2033-12-05', location: 'HQ',  category: 'Office Chair',subCategory: '', serialNumber: '', condition: AssetCondition.GOOD }
    ];
  }
}

interface MonthlyReportRow  { month: string; requested: number; approved: number; rejected: number; }
interface YearlyReportRow   { year:  number; requested: number; approved: number; rejected: number; }
interface TeamAssetHolding  { team: string; department: string; lead: string; totalAssets: number; hardware: number; software: number; peripheral: number; totalValue: number; members: number; }
interface TypeBreakdownItem { type: string; icon: string; color: string; bgColor: string; count: number; subCategories: { name: string; count: number }[]; }
